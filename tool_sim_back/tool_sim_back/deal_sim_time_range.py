...
def resample_ohlcv(df, timeframe='weekly'):
    """将日线OHLCV数据重采样为周线或月线"""
    ...
    freq = 'W-SUN' if timeframe == 'weekly' else 'M'

    agg_dict = {}
    for col in ['open', 'close', 'high', 'low', 'vol']:
        if col in df.columns:
            agg_dict[col] = {'open': 'first', 'close': 'last', 'high': 'max', 'low': 'min', 'vol': 'sum'}[col]
    ...
    print(f"  [重采样] {timeframe}: {len(df)} 条日线 -> {len(resampled)} 条{timeframe}线")
    return resampled
...
if __name__ == "__main__":
    data_folder = r'E:\gitClone\tool_sim\public\data'
    n_days = 20
